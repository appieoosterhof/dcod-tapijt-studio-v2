...
- **Gate:** `FloorDesign.status == "Bevestigd"` **én** `MaterialProfile.status == "Bevestigd"` **én** een **geldig** `SVGResultaat` (niet-leeg, identifier aanwezig) **én** een **niet-*stale* `Visualisatie`** (haar `SVGResultaat`-herkomst komt overeen met het aangeleverde `SVGResultaat`) **én** een **aanwezige, volledige Ontwerpvisie** (laag 1) **én** **interne herkomst-coherentie** (§4); anders geen bundeling (signalering).
...
- **Uitsluitend bevestigde/geldige invoer:** de gate (§2) toetst de bevestigde statussen van Floor Design en Material Profile, een geldig `SVGResultaat`, een niet-*stale* `Visualisatie`, en een aanwezige, volledige Ontwerpvisie. Laag 1 (Ontwerpvisie) draagt **geen eigen statusveld** (dat zit op het Concept, laag 4); de Builder toetst daarom **aanwezigheid/volledigheid** van de Ontwerpvisie — de bevestiging is contextueel (de bundeling gebeurt op het moment dat de architect tevreden is), niet een status óp laag 1.
