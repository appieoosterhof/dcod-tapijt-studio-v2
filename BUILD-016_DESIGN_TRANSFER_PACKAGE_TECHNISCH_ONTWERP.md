...
- **Uitsluitend bevestigde/geldige invoer:** de gate (§2) toetst de bevestigde statussen van Floor Design en Material Profile, een geldig `SVGResultaat`, een niet-*stale* `Visualisatie`, en een bevestigde, volledige Ontwerpvisie. Laag 1 (`OntwerpVisie`) draagt **geen string-status** (Voorgesteld/Bevestigd zit op het Concept, laag 4), maar wél een expliciete bevestigingsvlag **`bevestigd_door_architect: bool`**; de Builder toetst daarom die vlag (`is True`) **plus volledigheid** (ten minste één inhoudelijk visie-veld gevuld) — read-only, zonder de vlag ooit te zetten.
