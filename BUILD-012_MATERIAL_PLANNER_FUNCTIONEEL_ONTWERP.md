...
**Uitgangspunt:** de Material Planner is de eerste downstream-component ná de volledig geïmplementeerde Reasoning Engine. Hij **vormt uitsluitend materiaalvoorstellen** op basis van **het bevestigde Floor Design**, **bevestigt nooit**, wijzigt Concept noch Floor Design, en motiveert elk voorstel.

---

## 1. Plaats binnen de architectuur

De Material Planner is één van de planners die in de interne opbouw van de Design Brain ná de Reasoning Engine staan (DESIGN_BRAIN: Reasoning Engine → Material Planner / Pattern Planner / SVG Planner). Hij is **downstream** van de Reasoning Engine en werkt met **het door de architect bevestigde Floor Design** (uit Fase 2, ná bevestiging) (BUILD-007: "Floor Design → Material Profile: bepaald in samenhang met het al vastgestelde Floor Design — nooit andersom").

## 2. Functionele verantwoordelijkheid

De Material Planner **stelt materiaal voor, bevestigt nooit**. Hij vormt materiaalvoorstellen — de visuele en materiële eigenschappen van de vloerafwerking (uitstraling, structuur, pooltype; DESIGN_CONTEXT_MODEL laag 5, Materialisatie; BUILD-007) — in samenhang met **het bevestigde Floor Design** en het bevestigde Concept, en **motiveert** elk voorstel. Passend bij het gedeelde eigenaarschap van laag 5 (DCOD brengt materiaalkennis in, de architect stuurt op tactiliteit en uitstraling).

## 3. Input

- **Startvoorwaarde:** uitsluitend starten **nadat één Floor Design door de architect is bevestigd** (status "Bevestigd"; uit de Reasoning Engine, Fase 2).
- Het **bevestigde Floor Design** (read-only).
...
## 6. Gebruikte gegevens uit het bevestigde Floor Design

Uitsluitend lezend: de **ontwerprichting** van het bevestigde Floor Design en de **herkomst-referentie** naar het bevestigde Concept. Het materiaalvoorstel wordt in samenhang met die ontwerprichting gevormd; het Floor Design zelf wordt nooit gewijzigd.
...
- **Het bevestigde Concept niet wijzigen** en **het bevestigde Floor Design niet wijzigen**.
...
- **Upstream — Reasoning Engine + architect:** de Reasoning Engine produceert Floor Designs (Fase 2); de architect bevestigt er één. De Material Planner ontvangt **het bevestigde Floor Design** en (via de DesignContext) het bevestigde Concept. De scheiding blijft intact: de Material Planner produceert of wijzigt geen Floor Designs, de Reasoning Engine kent geen materiaal.
...
- Read-only op het Concept en het bevestigde Floor Design; geen enkele mutatie daarvan.
